...
size_t String_buffer_used_chunk_size;
size_t String_buffer_virtual_size;
size_t String_buffer_real_size;
  ...
  if (list->virtual_size == list->real_size)
    {
      ...
      *plist = list = (List) realloc(list, sizeof(struct ListStruct) + 
...
NewNode (KeyType key)
{
  Node x = (Node) malloc( sizeof(struct NodeStruct) );
  x->list = NewList();
  x->key = key;
  x->left = NULL;
  x->right = NULL;
...
dfs (Node x)
{
  if (x != NULL)
    {
      dfs(x->left);
      for (size_t i = 0; i < x->list->virtual_size; ++ i)
        printf("%s\n", x->list->array[i]);
      dfs(x->right);
    }
}
